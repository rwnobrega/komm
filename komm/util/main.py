...
def hexstr2binarray(hexstr, width):
    ...
    binstr = ''.join([f'{x:04b}' for x in intarray])
...
def entropy(pmf):
    ...
    return -pmf_nonzero @ np.log2(pmf_nonzero)


def mutual_information(input_pmf, transition_probabilities):
    output_pmf = input_pmf @ transition_probabilities
    entropy_output_prior = entropy(output_pmf)
    entropy_output_posterior = input_pmf @ np.apply_along_axis(entropy, 1, transition_probabilities)
